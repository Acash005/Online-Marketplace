...
def login():
    if request.method == 'POST':
        ...
        if role == 'admin':
            if(phone_no == '1234567890' and password == 'aakash@21'):
                return redirect(url_for("admin"))
            else:
                session['message'] = 'Wrong phone no or password'
                return redirect(url_for('login'))
        elif role == 'restaurant':
            q = f"SELECT * FROM res_details WHERE {role}_phone_no='{phone_no}';"
            mycursor.execute(q)
            ...
            session['message'] = 'Wrong password or Access Not Given yet'
            return redirect(url_for('login'))
        else:
...
def customerfood():
    if "user" in session:
        ...
        if(button_clicked == "food"):
            ...
            q = f"SELECT f.*, r.restaurant_name FROM food_details f JOIN res_details r ON f.res_id = r.restaurant_id WHERE restaurant_id = '{res_id}';"
            mycursor.execute(q)
            myresult = mycursor.fetchall()
            ...
            q = f"Select res_id from food_details where food_id = {food_id}"
            mycursor.execute(q)
            myresult = mycursor.fetchall()
            if food_id in order:
...
def orderplaced():
    if "user" in session:
        ...
        for x in order.keys():
            quant = 0
            q = f"Select * from food_details where food_id='{x}'"
            mycursor.execute(q)
            myresult = mycursor.fetchall()
            res_id = myresult[0][5]
            for i in order[x]:
        ...
        q = f"insert into orders(mode_of_payment, ord_amount, address, cust_id, w_id, res_id) values('{payment_mode}', '{amount}', '{result[0][1]}', '{result[0][0]}', '{w_id}', '{res_id}')"
        mycursor.execute(q)
        mydb.commit()
        order.clear()
...
def cust_orders():
    if "user" in session:
        ...
        q = f"SELECT o.*, r.restaurant_name FROM orders o JOIN res_details r ON o.res_id = r.restaurant_id WHERE o.cust_id = '{cust_id}';"
        mycursor.execute(q)
        myresult = mycursor.fetchall()
    ...
    if "user" in session:
        ...
        if(request.method == "GET"):
            ...
            if(button_clicked == "login"):
                q = f"SELECT f.*, r.restaurant_name FROM food f JOIN res_details r ON f.res_id = r.restaurant_id WHERE restaurant_id = '{res_id}';"
                mycursor.execute(q)
                myresult = mycursor.fetchall()
...
def addfood():
    if "user" in session:
        if(request.method == "POST"):
            ...
            q = f"SELECT * from res_details where restaurant_id = '{res_id}';"
            mycursor.execute(q)
            myresult = mycursor.fetchall()
...
def delivery_worker():
    if "user" in session:
        w_id = session["user"][0]
        if(request.method == "GET"):
            q = f"SELECT o.*, r.restaurant_name FROM orders o JOIN res_details r ON o.res_id = r.restaurant_id WHERE o.w_id = '{w_id}' and o.ord_status='On The Way';"
            mycursor.execute(q)
            myresult = mycursor.fetchall()
            print(myresult)
            ...
            mycursor.execute(q)
            mydb.commit()
            q = f"select w_id from orders where ord_id='{ord_id}';"
            mycursor.execute(q)
            myresult = mycursor.fetchall()
            q = f"update delivery_worker set no_of_deliveries = no_of_deliveries + 1 where delivery_worker_id='{myresult[0][0]}';"
            mycursor.execute(q)
            mydb.commit()
